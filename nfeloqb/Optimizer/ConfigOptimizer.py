...
class ConfigOptimizer:
    ...
    def __init__(
        ...
        self.subset: list[str] = subset if subset is not None else []
    ...
    def validate_objective(self):
        ...
        if self.objective_name not in ["mae", "mae_first_16", "mae_backup"]:
            raise ValueError(
                f"Objective {self.objective_name} is not a valid option returned by the scored "
                "record."
            )
    ...
    def denormalize_optimizer_values(self, x: list[float]) -> dict:
        ...
        for i, k in enumerate(x):
            local_config[self.features[i]] = self.denormalize_param(
                k, self.config.params[self.features[i]]
            )
        return local_config
    ...
    def objective(self, x: list[float]) -> float:
        ...
        # create the model
        model = QBModel(self.data, denormalized_config)
        # run the model
        model.run_model()
        # score the model
        ...
        if save_record:
            ...
            results_dir = pathlib.Path(__file__).parent.resolve() / "In-flight Results"
            date_str = datetime.datetime.now().strftime("%Y%m%d")
            subset_suffix = f"_{self.subset_name}" if len(self.subset) > 0 else ""
            file_path = f"{results_dir}/{date_str}{subset_suffix}.csv"
            df.to_csv(file_path)
