...
class Elo:
    ...
    def __init__(
        ...
        self.wt_ratings = pd.read_csv(f"{data_folder}/Manual Data/wt_ratings.csv")
        # model variables
        self.base = base
        self.k = k
        self.b = b
    ...
    def format_games(self):
        ...
        # reduce 2020 by 33
        hfa["hfa"] = numpy.where(hfa["season"] == 2020, hfa["hfa"] - self.no_fan, hfa["hfa"])
        # add to games
        self.game = pd.merge(self.game, hfa[["season", "hfa"]], on=["season"], how="left")
        # correct for neutrals
        self.game["hfa"] = numpy.where(self.game["location"] != "Home", 0, self.game["hfa"])
        # create byes
        self.game["home_bye"] = numpy.where(self.game["home_rest"] >= 10, 1, 0)
        self.game["away_bye"] = numpy.where(self.game["away_rest"] >= 10, 1, 0)
        # create by dif
    ...
    def handle_regression(self):
        ...
        for team, last_elo in self.current_elos.items():
            ...
            if len(temp) > 0:
                new_elo = (new_elo * (1 - self.reg_vegas)) + (
    ...
    def update_elos(self, record, home_elo, away_elo, home_prob, away_prob, elo_dif):
        ...
        mov_mult = numpy.log(abs(record["result"]) + 1) * (2.2 / (mov_elo_dif * 0.001 + 2.2))
        # calc new elos
    ...
    def run(self):
        ...
        # loop through games
        for _, record in self.game.iterrows():
