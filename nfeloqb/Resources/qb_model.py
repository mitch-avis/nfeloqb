...
class QBModel:
    ...
    def __init__(self, games: pd.DataFrame, model_config: ModelConfig):
        ...
        # storage for most recent team data
        self.teams: dict[str, Team] = {}
        # storage for weekly QB records over time#
        ...
        # track the time it takes to run the model
        self.model_runtime: float = 0
        # import original elo file location
        data_folder = pathlib.Path(__file__).parent.parent.resolve()
        self.original_file_loc = f"{data_folder}/Manual Data/original_elo_file.csv"
        # initial
        # sort by date, so games can be iter'd
        self.chrono_sort()
        self.add_averages()
    ...
    def get_objects(self, row) -> Tuple[QB, Team, Team]:
        ...
        * tuple - A tuple containing the QB and Team objects
        ...
        # QB
        if row["season"] > (
            qb.last_game_season if qb.last_game_season is not None else row["season"]
        ):
            qb.regress_value(
        ...
        # TEAM
        if row["season"] > (
            team.last_game_season_off if team.last_game_season_off is not None else row["season"]
        ):
        ...
        # OPPONENT
        if row["season"] > (
            opponent.last_game_season_def
            if opponent.last_game_season_def is not None
            else row["season"]
        ):
    ...
    def run_model(self):
        ...
        for _, row in self.games.iterrows():
            ...
            # get values
            qb_expected_value = qb.get_value(
                team
            )  # use get_value to account for the team adjustment
    ...
    def score_model(self, first_season=2009, add_elo=True):
        ...
        df["se"] = (
            df["qb_value_pre_def_adj"] - df["player_VALUE"]
        ) ** 2  # expectation for game including D, vs actual
        ...
        for roll in [8, 16, 24, 32]:
            df[f"se_r{roll}"] = (
                ...
                df.groupby(["player_id"])["player_VALUE"]
                .transform(lambda x, r=roll: x.rolling(r, min_periods=1).mean().shift())
                .fillna(df["qb_value_pre_def_adj"])
                -
                # actual value
                df["player_VALUE"]
            ) ** 2
            df[f"abs_error_r{roll}"] = numpy.absolute(
                # prediction based on a simple rolling average
                df.groupby(["player_id"])["player_VALUE"]
                .transform(lambda x, r=roll: x.rolling(r, min_periods=1).mean().shift())
                .fillna(df["qb_value_pre_def_adj"])
